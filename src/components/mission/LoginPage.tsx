...
export function LoginPage() {
  ...
  const [displayState, setDisplayState] = useState<AuthState>('PENDING');
  ...
  const handleSubmit = useCallback(
    (e: React.FormEvent) => {
      ...
      // Wait for bar to finish filling to 50% (0.5s delay + 1.5s animation = 2s)
      setTimeout(() => {
  ...
  const authStatusText =
    displayState === 'PENDING' ? 'PENDING'
      : displayState === 'VERIFYING' ? 'VERIFYING'
        : displayState === 'DENIED' ? 'DENIED'
          : 'GRANTED';

  const authSubtext =
    displayState === 'PENDING' ? 'AWAITING CLEARANCE INPUT'
      : displayState === 'VERIFYING' ? 'DECRYPTING CREDENTIALS...'
        : displayState === 'DENIED' ? 'UNAUTHORIZED ACCESS'
          : 'IDENTITY CONFIRMED';
    ...
    >
      ...
      <style>{`
        ...
        .para-input:focus {
          ...
          box-shadow: -4px 0 14px rgba(77,168,255,0.2), 0 0 8px rgba(77,168,255,0.1);
      ...
      <div
        ...
        style={{
          borderColor: `rgba(77,168,255,0.1)`,
      ...
      >
        <div
          ...
          style={{ fontFamily: "var(--font-orbitron), sans-serif", color: P.blue }}
        >
          XLR8 AUTOMATA-IV // RECURSION NODE // FINALS
        </div>
        <div className="hidden sm:flex items-center gap-4">
          <StatusDot label="HULL INTACT" color="green" />
          <StatusDot label="CREW: 0/4" color="gold" />
          <StatusDot label="COMMS LIVE" color="cyan" pulse />
        </div>
      </div>

      {/* ── Main Content: Two Column Layout — fills viewport ──────────── */}
      <div className="relative z-10 flex-1 flex flex-col lg:flex-row items-center justify-center gap-0 lg:gap-4 overflow-hidden">
        ...
        <motion.div
          ...
          className="flex flex-col items-center w-full lg:w-auto py-2 px-3 sm:px-5 lg:px-6 overflow-visible"
        >

          {/* Radar SVG — 260px */}
          <div className="relative shrink-0" style={{ width: 250, height: 250, margin: '15px' }}>
            <div
          ...
          <div className="w-full grid grid-cols-2 gap-2.5 mt-5 flex-1" style={{ maxWidth: 300 }}>
            ...
            <div className="col-span-2">
              <GridCell
                ...
                barColor={
                  displayState === 'GRANTED' ? P.success
                    : displayState === 'VERIFYING' ? P.gold
                      : displayState === 'DENIED' ? P.error
                        : P.purple
                }
                accentColor={
                  displayState === 'GRANTED' ? P.success
                ...
                statusDot={
                  displayState === 'VERIFYING' ? 'pulse' as const
                    : displayState === 'DENIED' ? 'fast' as const
                      : 'none' as const
                }
                fullWidth
              />
        ...
        <motion.div
          ...
          className="flex flex-col items-center lg:items-start w-full lg:w-auto max-w-[400px] py-4 px-4 sm:px-6 lg:px-8 overflow-hidden"
        >
          {/* Orbit Sector — above subtitle */}
          <div className="text-center lg:text-left shrink-0 mb-1">
            <div
              className="text-[11px] tracking-[0.3em] uppercase"
              style={{ color: P.blue, fontFamily: "var(--font-orbitron), sans-serif" }}
            >
              Orbit Sector
            </div>
            <div
              className="text-[20px] font-bold tracking-[0.1em] uppercase"
              style={{ color: P.blue, fontFamily: "var(--font-orbitron), sans-serif" }}
            >
          ...
          {/* Subtitle */}
          <div
            className="text-[10px] tracking-[0.2em] uppercase mb-1.5 text-center lg:text-left shrink-0"
            style={{ fontFamily: "var(--font-orbitron), sans-serif", color: P.purple }}
          >
          ...
          {/* Title with glitch */}
          <div
            className="text-[35px] font-bold tracking-[0.08em] uppercase text-center lg:text-left mb-2 shrink-0"
            style={{
          ...
          >
            <span
              style={{
                color: P.primaryHov,
                textShadow: `0 0 20px rgba(0,245,255,0.5), 0 0 40px rgba(0,245,255,0.15)`,
                position: 'relative',
          ...
          <div className="w-full flex items-center mb-2 shrink-0">
            <div className="w-1.5 h-1.5 rounded-full mr-2"
              style={{ backgroundColor: P.green, boxShadow: `0 0 4px ${P.green}` }} />
            <span className="text-[11px] tracking-[0.1em]" style={{ color: P.gold }}>
          ...
          <div className="w-full flex items-center gap-2 mb-3 shrink-0">
            ...
            <div className="w-1.5 h-1.5 rotate-45" style={{ backgroundColor: P.gold, opacity: 0.4 }} />
            <div className="flex-1 h-px" style={{ background: `rgba(0,206,201,0.12)` }} />
          </div>

          {/* Mission Brief */}
          <div
            className="w-full border rounded p-3 mb-3 shrink-0 relative overflow-hidden"
            style={{
          ...
          >
            {/* Top accent bar */}
            <div className="absolute top-0 left-0 right-0 h-[2px]" style={{ background: P.purple, boxShadow: `0 0 8px ${P.purple}60` }} />
            <div
            ...
            <p className="text-[12px] leading-relaxed" style={{ color: P.textSec }}>
              XLR8 Vessel inbound on Orbit-IV Lab Node. Docking corridor requires biometric-level crew verification. Authenticate to proceed.
            </p>
          </div>

          {/* Form — flex-1 to fill remaining space */}
          <form onSubmit={handleSubmit} className="w-full flex flex-col flex-1 gap-1">
            {/* Crew Identifier */}
            <div>
              <label
            ...
            <div>
              <label
                className="block text-[10px] tracking-[0.15em] uppercase mb-1"
                style={{ color: P.purple, fontFamily: "var(--font-orbitron), sans-serif" }}
              >
                Encryption Key
              </label>
              <div className="relative">
                ...
                <button
                  ...
                  style={{ color: '#00D2FF', transition: 'color 0.2s' }}
            ...
            <button
              ...
              className="para-btn w-full py-3 mt-2 text-xs tracking-[0.18em] uppercase cursor-pointer"
              onMouseEnter={() => { if (soundEnabled) soundEngine.playHover(); }}
              style={{
                fontFamily: "var(--font-orbitron), sans-serif",
                color: displayState === 'GRANTED' ? P.success : displayState === 'DENIED' ? P.error : displayState === 'VERIFYING' ? P.gold : '#050B18',
                background: displayState === 'GRANTED' ? `rgba(125,249,192,0.15)`
                  : displayState === 'DENIED' ? `rgba(255,59,48,0.15)`
                  : displayState === 'VERIFYING' ? `rgba(245,200,66,0.08)` : `linear-gradient(135deg, ${P.primaryHov}, ${P.primary})`,
                border: `1px solid ${displayState === 'GRANTED' ? `rgba(125,249,192,0.4)`
                  : displayState === 'DENIED' ? `rgba(255,59,48,0.4)`
                  : displayState === 'VERIFYING' ? `rgba(245,200,66,0.3)` : `rgba(0,245,255,0.3)`}`,
                animation: displayState === 'GRANTED' ? 'grantedGlow 1s ease-in-out'
                  : displayState === 'DENIED' ? 'deniedShake 0.4s ease'
                  : displayState === 'PENDING' ? 'breathe 3s ease-in-out infinite' : 'none',
                opacity: 1,
              }}
            >
              {displayState === 'PENDING' && 'INITIATE CLEARANCE'}
              {displayState === 'VERIFYING' && (
                <span style={{ animation: 'verifyingPulse 1s ease-in-out infinite' }}>VERIFYING...</span>
              )}
              {displayState === 'GRANTED' && 'CLEARANCE GRANTED'}
              {displayState === 'DENIED' && 'ACCESS DENIED'}
            </button>
          </form>
...
function StatusDot({ label, color, pulse }: { label: string; color: string; pulse?: boolean }) {
  const dotColor = color === 'cyan' ? P.primary : color === 'green' ? P.green : color === 'gold' ? P.gold : color === 'purple' ? P.purple : P.blue;
  const labelColor = color === 'cyan' ? `rgba(0,206,201,0.5)` : color === 'green' ? `rgba(125,249,192,0.5)` : color === 'gold' ? `rgba(245,200,66,0.5)` : color === 'purple' ? `rgba(123,111,255,0.5)` : `rgba(77,168,255,0.5)`;
  return (
    <div className="flex items-center gap-1.5">
      ...
      <span
        ...
        style={{ fontFamily: "var(--font-orbitron), sans-serif", color: labelColor }}
      >
        {label}
      </span>
    </div>
